...
try:
    ...
    with open('birst_config.txt') as f:
        for line in f:
            ...
            if line.startswith('urls'):
                url_list = line.partition('=')[2].strip().split(',')
                url_list1 = [x.strip() for x in url_list]

            if line.startswith('regions'):
                region_list = line.partition('=')[2].strip().split(',')
                region_list1 = [x.strip() for x in region_list]

            if line.startswith('guids'):
                guids = line.partition('=')[2].strip().split(',')
                guids1 = [x.strip() for x in guids]

                for i in range(len(url_list1)):
                    admin_get_valid_base_urls()[url_list1[i]] = [region_list1[i], guids1[i]]
    ...
    if not url_list:
        exit_code = 2
        raise RuntimeError("URLs not given in the config file. Missing tag: urls")

    if not region_list:
        exit_code = 2
        raise RuntimeError("Regions not given in the config file. Missing tag: regions")

    if not guids:
        exit_code = 2
        raise RuntimeError("guids not given in the config file. Missing tag: guids")
...
try:
    ...
    logger.info('Login to ' + download_url + ' Successful')
    ...
    if space_prefix:
        ...
        download_spaces = copy_config[copy_from_spaces_def]

    download_spaces.sort(key=str.lower)
    # print(download_spaces)
    ...
    # no need of admin web service directly can use getSpaceStatus WebService
    ...
    for num, space_to_download in enumerate(download_spaces, start=1):
        ...
        if space_prefix:
            ...
            space_name = space_to_download.strip()
